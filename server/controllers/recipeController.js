...
const getRecipes = async (req, res) => {
    try {
        ...
        if (req.user && !sort) {
            ...
            const userDiet = user.dietaryPreference;

            recipes = recipes.map(recipe => {
                let score = 0;

                // Boost followed users
                if (followingIds.includes(recipe.user._id.toString())) {
                    score += 10;
                }

                // Boost dietary match
                if (userDiet === 'veg' && recipe.isVegetarian) score += 5;
                if (userDiet === 'non-veg' && !recipe.isVegetarian) score += 5;

                // Recency boost (decay over time)
                const hoursAge = (Date.now() - recipe.createdAt) / (1000 * 60 * 60);
                score += Math.max(0, 10 - (hoursAge / 24)); // Up to 10 points for very recent
            ...
            // If we didn't personalize, we need to apply sort manually or re-query.
            // Since we already fetched, let's sort in JS to be consistent.
            if (sort === 'popular') {
                recipes.sort((a, b) => (b.likesCount || 0) - (a.likesCount || 0));
            } else if (sort === 'trending') {
